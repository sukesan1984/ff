...
var craft_hp_max := 3
var craft_drill := 0    # 採掘範囲拡大
var craft_magma := 0    # マグマ耐性(溶岩無効)
var mobs: Array[Mob] = []
var _mob_cd := 2.0
var chain := 0          # チェイン採掘コンボ
var _chain_t := 0.0

const CRAFT_DEFS := [
	{"id": "c_pick", "name": "ツルハシ強化", "short": "鶴", "desc": "硬い扉を掘れる(進行に必須)"},
	{"id": "c_armor", "name": "よろい", "short": "鎧", "desc": "ハート最大+1＆全回復"},
	{"id": "c_wings", "name": "つばさ", "short": "翼", "desc": "落下が遅く操作しやすい"},
	{"id": "c_drill", "name": "ドリル", "short": "錐", "desc": "採掘範囲が広がる(掘りやすい)"},
	{"id": "c_magma", "name": "マグマブーツ", "short": "炎", "desc": "溶岩のダメージを無効化"},
]
...
func _craft_update(delta: float, dx: float) -> void:
	...
	if _chain_t <= 0.0:
		chain = 0
	# 採掘(ツルハシLvで掘れる硬さ、ドリルで範囲)
	var mr := cur_radius + 7.0 + craft_drill * 13.0
	...
	for t in result["types"]:
		...
		# チェイン採掘:掘り続けると倍率(止まると消える)
		chain += 1
		_chain_t = 1.1
		var cmul := 1.0 + minf(chain, 30) * 0.1
		_craft_ore += int(round(gain * cmul))
		_add_fever(0.07)
	if chain >= 5 and not result["types"].is_empty():
		_floater("チェイン x%d!" % chain, bird.position + Vector2(0, -64), Color(1, 0.8, 0.3), 24)
...
func _update_mobs(delta: float, dx: float) -> void:
	# 出現(深さに応じて)
	_mob_cd -= delta
	if _mob_cd <= 0.0 and craft_depth > 12 and mobs.size() < 4:
		_mob_cd = randf_range(2.0, 4.0)
		var m := Mob.new()
		m.kind = Mob.BAT if randf() < 0.4 else Mob.CREEPER
		m.base_y = randf_range(blocks.top_y + 30.0, GROUND_Y - 80.0)
		m.position = Vector2(W + 50, m.base_y)
		world.add_child(m)
		mobs.append(m)
	for m in mobs.duplicate():
		m.position.x -= dx * (0.95 if m.kind == Mob.BAT else 0.85)
		# コウモリは鳥へ寄る
		if m.kind == Mob.BAT:
			m.base_y = lerpf(m.base_y, bird.position.y, delta * 1.4)
		m.tick(delta)
		var d := bird.position.distance_to(m.position)
		if m.kind == Mob.CREEPER:
			# 近づくと点火→爆発
			if not m.lit() and d < 110.0:
				m.fuse = 0.9
				sfx.play("nice", 0.7)
			if m.lit() and m.fuse <= 0.0:
				_creeper_explode(m)
				continue
		else:
			if d < cur_radius + Mob.RADIUS and invuln_t <= 0.0 and not fever_active:
				_craft_take_damage("コウモリ！")
				m.dead = true
		if m.position.x < -60.0 or m.dead:
			mobs.erase(m)
			m.queue_free()


func _creeper_explode(m: Mob) -> void:
	mobs.erase(m)
	var ores := blocks.blast(m.position, 2)  # 周囲を吹き飛ばす(道が開く)
	for t in ores:
		if t == Blocks.DIAMOND:
			res["g"] += 1
		else:
	...
	_burst(m.position, Color(1, 0.7, 0.2), 32, 320.0, 0.7, 4.5)
	# 近ければ被弾
	if bird.position.distance_to(m.position) < 130.0 and invuln_t <= 0.0 and not fever_active:
		_craft_take_damage("爆発に巻き込まれた！")
	m.queue_free()
...
func _build_levelup() -> void:
	...
	level_title = _mk_label(level_box, "★ レベルアップ！ ★", 244, 46, Color(1, 0.9, 0.35))
...
func _craft_maxed(id: String) -> bool:
	...
	if id == "c_drill":
		return craft_drill >= 3
	if id == "c_magma":
		return craft_magma >= 1
	return false
...
func _do_craft(id: String) -> void:
	...
	match id:
		...
		"c_drill":
			craft_drill += 1
			_floater("ドリル強化！採掘範囲UP", Vector2(W * 0.5, H * 0.42), Color(0.7, 0.9, 1), 28)
		"c_magma":
			craft_magma += 1
			_floater("マグマブーツ！溶岩を無効化", Vector2(W * 0.5, H * 0.42), Color(1, 0.7, 0.4), 28)
	sfx.play("powerup")
	_hit_stop(0.06)
...
func _offer_craft() -> bool:
	if _leveling or state != PLAY:
		return false
	# 5種から3枚を抽選(ツルハシは進行に必須なので未MAXなら優先で1枠)
	var pick_list: Array = []
	var rest: Array = []
	for d in CRAFT_DEFS:
		if str(d["id"]) == "c_pick" and not _craft_maxed("c_pick"):
			pick_list.append(d)
		else:
			rest.append(d)
	rest.shuffle()
